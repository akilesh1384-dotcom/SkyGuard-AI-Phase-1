...
export const sensorReadingsTable = pgTable(
  ...
  (table) => ({ timestampUnique: uniqueIndex("skyguard_sensor_readings_timestamp_unique").on(table.timestamp) }),
);
...
export const insertSimulatorEventSchema = createInsertSchema(simulatorEventsTable).omit({ id: true });
