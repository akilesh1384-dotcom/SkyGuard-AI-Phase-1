...
router.get("/ml/readings", async (req, res): Promise<void> => {
  ...
  const baseQuery = db.select({ timestamp: sensorReadingsTable.timestamp, temperature: sensorReadingsTable.temperature, humidity: sensorReadingsTable.humidity, pressure: sensorReadingsTable.pressure }).from(sensorReadingsTable).orderBy(asc(sensorReadingsTable.timestamp));
...
router.get("/ml/ground-truth", async (_req, res): Promise<void> => {
  const events = await db.select({ fault_type: simulatorEventsTable.faultType, affected_variable: simulatorEventsTable.affectedVariable, start_timestamp: simulatorEventsTable.startTimestamp, end_timestamp: simulatorEventsTable.endTimestamp }).from(simulatorEventsTable).orderBy(asc(simulatorEventsTable.startTimestamp));
...
router.get("/ml/analyze", async (req, res): Promise<void> => {
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 200) { res.status(400).json({ error: "limit must be an integer between 1 and 200" }); return; }
  try {
    ...
    res.status(503).json({ status: "unavailable", error: "ML service is unavailable", detail: error instanceof Error ? error.message : String(error) });
  }
});
...
router.post("/simulator/stop", async (_req, res): Promise<void> => { res.json(StopSimulatorResponse.parse(await skyguardSimulator.stop())); });
router.post("/simulator/reset", async (_req, res): Promise<void> => { res.json(ResetSimulatorResponse.parse(await skyguardSimulator.reset())); });
router.post("/simulator/mode", async (req, res): Promise<void> => {
  ...
  if (!Object.values(SimulatorMode).includes(mode)) { res.status(400).json({ error: "Invalid simulator mode" }); return; }
