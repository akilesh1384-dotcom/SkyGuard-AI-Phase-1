...
export default function AiAnalysisPanel() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch(`/api/ml/analyze?limit=1&sensor_mode=${mode}`, { cache: 'no-store' });
        if (!response.ok) throw new Error(`ML API ${response.status}`);
        const payload = (await response.json()) as AnalysisResponse;
        if (!disposed) { setAnalysis(payload); setError(false); }
      } catch { if (!disposed) setError(true); }
      finally { if (!disposed) timer = window.setTimeout(load, 5000); }
    };
    load();
  ...
  return createPortal(
    <section className="w-full overflow-hidden rounded-[1.15rem] border border-[#356872] bg-[#123541] text-[#eef8f5] shadow-[0_12px_35px_rgba(12,42,52,.16)]" data-testid="panel-ai-analysis">
      ...
      {ready && result && <div className="px-5 pb-5 pt-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 rounded-xl bg-[#102f39]/80 px-3 py-2.5 text-[10px] text-[#9fc0c0]"><span>Mode <strong className="text-[#e9f6f3]">{mode}</strong></span><span>Alert <strong className="text-[#e9f6f3]">{active ? result.alert_state : 'NORMAL'}</strong></span><span>Final score <strong className="text-[#e9f6f3]">{percent(result.final_score)}</strong></span><span>Baseline <strong className="text-[#e9f6f3]">{analysis.baseline_progress}/{analysis.baseline_required}</strong></span></div>
