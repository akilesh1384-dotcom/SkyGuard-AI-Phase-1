...
type ShapContribution = { feature: string; value: number; shap_value: number; direction: string; };
type AnomalyResult = { timestamp: string; statistical_score: number; ml_score: number; diagnostic_score: number; multivariate_score: number; final_score: number; is_anomaly: boolean; alert_active: boolean; alert_state: string; diagnostic_anomaly: boolean; fault_type: string; affected_variable: string | null; reasons: string[]; shap_base_value?: number | null; shap_contributions?: ShapContribution[]; };
...
function shapTone(value: number) { return value < 0 ? 'text-[#ff9b92]' : 'text-[#63e2cc]'; }
...
export default function AiAnalysisPanel() {
  ...
  const shapContributions = result?.shap_contributions ?? [];
  const topShap = shapContributions.slice(0, 2);
  const maxAbsShap = Math.max(...shapContributions.map((item) => Math.abs(item.shap_value)), 1e-9);
  ...
  return createPortal(
    <section className="w-full overflow-hidden rounded-[1.15rem] border border-[#356872] bg-[#123541] text-[#eef8f5] shadow-[0_12px_35px_rgba(12,42,52,.16)]" data-testid="panel-ai-analysis">
      ...
      {ready && result && <div className="px-5 pb-5 pt-4">
        ...
        {topShap.length > 0 && <div className="mt-3 rounded-xl border border-[#315d65] bg-[#173d48]/70 p-3"><div className="mb-2 text-[9px] font-bold uppercase tracking-[0.13em] text-[#7da4a7]">Top SHAP contributors</div><div className="grid gap-2 sm:grid-cols-2">{topShap.map((item) => <div key={item.feature} className="rounded-lg bg-[#102f39]/80 p-2.5"><div className="flex items-center justify-between gap-2 text-[10px]"><span className="font-semibold text-[#d8ece9]">{faultLabel(item.feature)}</span><span className={`font-data font-bold ${shapTone(item.shap_value)}`}>{item.shap_value > 0 ? '+' : ''}{item.shap_value.toFixed(3)}</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[#294f58]"><div className={`h-full rounded-full ${item.shap_value < 0 ? 'bg-[#ef7168]' : 'bg-[#45d5c1]'}`} style={{ width: `${Math.max(4, Math.min(100, Math.abs(item.shap_value) / maxAbsShap * 100))}%` }} /></div><div className="mt-1 text-[9px] text-[#8fb3b4]">Sensor value: <strong className="text-[#cbe1df]">{item.value.toFixed(2)}</strong> · {item.direction === 'increases_anomaly' ? 'pushes toward anomaly' : 'pushes away from anomaly'}</div></div>)}</div></div>}
